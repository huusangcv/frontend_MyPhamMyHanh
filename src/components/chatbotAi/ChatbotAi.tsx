...
const ChatbotAi = () => {
  ...
  const handleAskChatbot = async () => {
    ...
    setAsk('');
    try {
      const { status, data } = await chatbotAiMethods.createContent(ask as string);
      if (status) {
                  ...
                  history.map((historyItem, index) => (
                    <>
                      <div className={cx('message-bot')} key={index}>
                        ...
                        />
                        <div className={cx('message-content')}>{historyItem.content}</div>
                      </div>
                      <div className={cx('message-user')}>
                        <div className={cx('message-content')}>{historyItem.ask}</div>
