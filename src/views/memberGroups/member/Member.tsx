...
const getMembershipLevel = (totalSpent: number) => {
  if (totalSpent >= 10000000) return 'KHÁCH HÀNG VIP';
  if (totalSpent >= 5000000) return 'KHÁCH HÀNG THÂN THIẾT';
  if (totalSpent >= 1000000) return 'KHÁCH HÀNG BẠC';
...
const Member = () => {
  ...
  const membershipLevel = getMembershipLevel(totalMoney);
    ...
    <div className={cx('wapper')}>
      <div className={cx('block-info')}>
        <div className={cx('block-mem-info__left')}>
          <div className={cx('block-welcome')}>
            <div className={cx('block-welcome__box-content')}>
              <Avatar
                src={profile?.image ? `http://res.cloudinary.com${profile.image}` : undefined}
              ...
              <div className={cx('welcome-member')}>
                ...
                <div className="d-flex align-items-center">
                  ...
                  <svg
                    ...
                    viewBox={showPhone ? '0 0 640 512' : '0 0 576 512'}
                ...
                <div className={cx('label-rank')}>
                  <p className={cx('welcome-member__level')}>{membershipLevel}</p>
                </div>
              </div>
            </div>
          </div>
              ...
              {loadingOrders ? (
                ...
                <>
                  <div className={cx('content__item')}>
                    <p className={cx('item__content', 'title')}>{completedOrders}</p>
                    <p className={cx('item__content', 'text')}>Đơn hàng đã hoàn thành</p>
                  </div>
                  <div className={cx('content__item')}>
                    <p className={cx('item__content', 'title')}>{pendingOrders}</p>
                    <p className={cx('item__content', 'text')}>Đơn hàng đang xử lý</p>
                  </div>
                  <div className={cx('content__item')}>
        ...
        <div className={cx('block-smem-info__right')}>
          <div className={cx('block-homepage-menu')}>
            <div className={cx('block-homepage-menu__title')}>
              Nhận mã xác thực và tận hưởng ưu đãi chiết khấu đặc quyền!
            </div>
            <div className={cx('block-homepage-menu__content')}>
              <div className={cx('is-qr-code')}>
                <img src={qrCode} alt="QR Code" loading="lazy" className={cx('is-qr-code')} />
              </div>
            </div>
            <div className={cx('block-homepage-menu__bottom')}>
              <div className={cx('bottom-timer')}>Quét để nhận ưu đãi khi mua sắm</div>
            </div>
          </div>
        </div>
      </div>
