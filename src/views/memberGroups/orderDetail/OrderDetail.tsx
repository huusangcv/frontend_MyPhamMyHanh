...
const OrderDetail = () => {
  ...
  const handleBuyBack = async (productId: string) => {
    try {
      const { data, status } = await productMethods.getDetailProductById(productId);
      if (status) {
        dispatch(
          addItemToPayment({
            id: data._id,
        ...
        <div className={cx('order-detail')}>
          <div className={cx('top-nav-bar')}>
            <div className={cx('navbar-container')}>
              <div>
                <div data-v-5170e23d="" className={cx('back-icon')} onClick={handleBack}>
                                    ...
                                    <div className={cx('info__group')}>
                                      {order.status === 'delivered' && (
                                        <div className={cx('group-btn-info')}>
                                          ...
                                          <div className={cx('btn-info')} onClick={() => handleBuyBack(product.id)}>
                                            Mua lại
                                          </div>
                                        </div>
                                      )}
                  ...
                  <div className={cx('payment-info__content')}>
                    ...
                    {(order.isPaid && (
                      <div className={cx('content__item', 'last-item-money')}>
                        <p className={cx('item-title', 'green')}>Đã thanh toán:</p>
                        <p className={cx('has-text-weight-bold', 'green')}>{formatter.format(order.paidAmount)}đ</p>
                      </div>
                    )) || (
                      <div className={cx('content__item', 'last-item-money')}>
