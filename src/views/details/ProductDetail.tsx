...
const ProductDetail = () => {
  ...
  const sameProducts = products.filter(
    (sameProduct) => sameProduct.category_id === product.category_id && sameProduct._id !== product._id,
  );
          ...
          <section className={cx('block-detail-product')}>
            ...
            <div className={cx('block-detail-product', 'wrap')}>
              <Grid container spacing={2}>
                <Grid item md={7} xs={12} className={cx('box-detail-product__box-left')}>
                  <div className={cx('box-gallery')}>
                    <div className={cx('gallery-product-detail')}>
                      <div className={cx('gallery-slide')}>
                        <div className={cx('thumbnail-slide', 'swiper-container')}>
                          <div className={cx('swiper-slide')}>
                            <img
                              className={cx('img')}
                              src={`https://https://backend.regis.id.vn${product.images[imageIndex]}`}
                              alt=""
                            />
                          </div>
                        </div>
                      </div>
                      <div className={cx('thumbnail-slide', 'swiper-container', 'swiper-container-thumbs', 'footer')}>
                        {product.images.map((image, index) => (
                          <div
                            className={cx('swiper-slide', imageIndex === index && 'swiper-slide-thumb-active')}
                            key={index}
                            onClick={() => setImageIndex(index)}
                          >
                            <img className={cx('img')} src={`https://https://backend.regis.id.vn${image}`} alt="" />
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <Grid item md={5} xs={12} className={cx('box-detail-product__box-center')}>
                      ...
                      return (
                        <div className={cx('sforum__content')} key={news._id}>
                          <Link to={`/news/detail/${news.slug}`} className={cx('sforum__content-item')}>
                            <img
                              src={`https://https://backend.regis.id.vn${news.image}`}
                              alt={news.title}
                              loading="lazy"
                              className={cx('content-item__img')}
                            />
                            <div className={cx('content-item__text')}>{news.title}</div>
                          </Link>
                        </div>
                      );
                    }
          ...
          <Grid container spacing={2}>
            <Grid item md={8} xs={12}>
              <div className={cx('boxReview')}>
                <h2 className="title is-6">Đánh giá &amp; nhận xét {product.name}</h2>
